"""
Indexing Script for mrtidy Dataset
...
def parse_args():
    parser = argparse.ArgumentParser(description="Index mrtidy corpus")
...
async def main(exp: dict):
    ...
    # Load corpus from local HuggingFace dataset
    _LANG_FULL = {"en": "english", "ko": "korean"}
    lang_full = _LANG_FULL.get(lang, lang)
    corpus_path = Path(settings.data_dir) / f"{lang_full}-corpus"
    print(f"Loading mrtidy corpus for '{lang}' from {corpus_path}...")

    corpus_ds = load_from_disk(str(corpus_path))["train"]
    if max_docs:
        corpus_ds = corpus_ds.select(range(min(max_docs, len(corpus_ds))))

    docs = [
        Document(
            page_content=row["text"][:max_length],
            metadata={"docid": row["_id"]},
        )
