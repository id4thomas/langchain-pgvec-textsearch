"""
Indexing Script for MLDR Dataset
...
def parse_args():
    parser = argparse.ArgumentParser(description="Index MLDR corpus")
...
async def main(exp: dict):
    ...
    # Load corpus from local JSONL
    corpus_path = Path(settings.data_dir) / f"mldr-v1.0-{lang}" / "corpus.jsonl"
    print(f"Loading MLDR corpus for '{lang}' from {corpus_path}...")
    ...
    with open(corpus_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            docs.append(
                Document(
                    page_content=x["text"][:max_length],
                    metadata={"docid": x["docid"]},
                )
            )
