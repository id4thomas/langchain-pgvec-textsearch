# %%
"""
Retrieval Evaluation Script for MLDR Dataset (English)

Uses:
- English queries from qrels TSV files
...
# %%
def load_queries(data_dir: str, lang: str, split: str = "dev") -> dict[str, str]:
    """Load queries from JSONL.gz file."""
    queries_path = Path(data_dir) / f"mldr-v1.0-{lang}" / f"{split}.jsonl.gz"
    queries = {}
    with gzip.open(queries_path, "rt", encoding="utf-8") as f:
        for line in f:
            item = json.loads(line)
            queries[item["query_id"]] = item["query"]
    return queries


def load_qrels(data_dir: str, lang: str, split: str = "dev") -> dict[str, set[str]]:
    """Load qrels (relevance judgments) from TSV file."""
    qrels_path = Path(data_dir) / "qrels" / f"qrels.mldr-v1.0-{lang}-{split}.tsv"
...
def load_evaluation_data(data_dir: str, lang: str, split: str = "dev"):
    ...
    queries_dict = load_queries(data_dir, lang, split)
    qrels_dict = load_qrels(data_dir, lang, split)
...
async def main(k_values: list[int] = [5, 10, 20], split: str = "test"):
    ...
    print(f"=== MLDR English Evaluation (text_config: {TEXT_CONFIG}) ===")
...
def print_results(all_results: dict, k_values: list[int]):
    ...
    print(f"\n{'='*70}")
    print(f"MLDR English EVALUATION RESULTS")
    print(f"{'='*70}")
