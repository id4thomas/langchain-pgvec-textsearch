# %%
"""
Retrieval Evaluation Script for MLDR Dataset
...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate retrieval on MLDR")
...
def load_evaluation_data(data_dir: str, lang: str, split: str = "dev"):
    """Load topics and qrels for MLDR evaluation."""
    base_path = Path(data_dir)
    topics_file = base_path / f"mldr-v1.0-{lang}" / f"{split}.jsonl"
    qrels_file = base_path / "qrels" / f"qrels.mldr-v1.0-{lang}-{split}.tsv"
    queries_dict = load_queries(str(topics_file))
...
async def main(index_cfg: dict, eval_cfg: dict):
    ...
    print(f"=== MLDR Evaluation (text_config: {text_config}) ===")
