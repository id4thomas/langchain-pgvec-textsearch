...
def load_data(data_dir: str, split: str = "dev", query_lang: str = "ko", corpus_lang: str = "ko"):
    """Load qrels, queries, and corpus."""
...
def main(num_samples: int = 500):
    ...
    qrels_dict, queries_dict, corpus_dict, corpus_ids = load_data(
        settings.data_dir, split="dev", query_lang="ko", corpus_lang="ko"
    )
    ...
    plt.title(f"Cosine Similarity Distribution (OpenAI Embedding: {settings.embedding_model})\n"
              f"Ko-StrategyQA Dataset (n={len(sampled_query_ids)})")
