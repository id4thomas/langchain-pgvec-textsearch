# %%
"""
Retrieval Evaluation Script for Ko-StrategyQA Dataset (English)

Uses:
- English queries (en split)
...
NOTE: This script requires a separate table indexed with English corpus.
Run 2-index-en.py first to create the English index.
"""
import asyncio
import numpy as np
...
TABLE_NAME = f"{DATA_NAME}-documents-en"  # Table with English corpus and english BM25 index
QUERY_LANG = "en"  # English queries
...
async def main(k_values: list[int] = [5, 10, 20], split: str = "dev"):
    ...
    print(f"=== English Evaluation (text_config: {TEXT_CONFIG}) ===")
...
def print_results(all_results: dict, k_values: list[int]):
    ...
    print(f"\n{'='*70}")
    print(f"ENGLISH EVALUATION RESULTS")
    print(f"{'='*70}")
