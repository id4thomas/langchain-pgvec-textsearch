# %%
"""
Retrieval Evaluation Script for markers_bm Dataset (Korean)

Uses:
- Korean queries (queries split)
...
QUERY_SPLIT = "queries"  # Korean queries
...
# %%
def load_evaluation_data(data_dir: str, split: str = "test", query_split: str = "queries"):
    ...
    queries = queries_ds[query_split]
...
async def main(k_values: list[int] = [5, 10, 20], split: str = "test"):
    ...
    print(f"Query split: {QUERY_SPLIT}")
    ...
    qrels_dict, queries_dict = load_evaluation_data(
        settings.data_dir, split, query_split=QUERY_SPLIT
    )
...
def print_results(all_results: dict, k_values: list[int]):
    ...
    print(f"\n{'='*70}")
    print(f"markers_bm EVALUATION RESULTS")
    print(f"{'='*70}")
